...
for ind = 1:length(DType)
    ...
    D.(DType{ind}).labels = rand(1, D.(DType{ind}).N) >= priors(1);
    ...
    [D.(DType{ind}).x(:, ~D.(DType{ind}).labels),D.(DType{ind}).dist(:, ~D.(DType{ind}).labels)] =randGMM(D.(DType{ind}).N0, w0, mu0, Sigma0);
    
    [D.(DType{ind}).x(:, D.(DType{ind}).labels),D.(DType{ind}).dist(:, D.(DType{ind}).labels)] =randGMM(D.(DType{ind}).N1, w1, mu1, Sigma1);
    
    subplot(2, 2, ind);
    plot(D.(DType{ind}).x(1, ~D.(DType{ind}).labels),D.(DType{ind}).x(2, ~D.(DType{ind}).labels), 'c.', 'DisplayName', 'Class 0');
    hold all;
    plot(D.(DType{ind}).x(1, D.(DType{ind}).labels),D.(DType{ind}).x(2, D.(DType{ind}).labels), 'm.', 'DisplayName', 'Class 1');
    grid on;
    xlabel('x1');
    ylabel('x2');
...
% Part 1: 
if Part1
    px0 = evalGMM(D.d20k.x, w0, mu0, Sigma0);
    px1 = evalGMM(D.d20k.x, w1, mu1, Sigma1);
    discriminantScore = log(px1 ./ px0);
    sortDS = sort(discriminantScore);
    logGamma = [min(discriminantScore) - eps, sort(discriminantScore) + eps];
    prob = CalcProb(discriminantScore, logGamma, D.d20k.labels, D.d20k.N0, D.d20k.N1, D.d20k.phat);
    logGamma_ideal = log(priors(1) / priors(2));
    ...
    hold all;
    plot(p10_ideal, p11_ideal, 'm+', 'DisplayName', 'Ideal Min. Error');
    plotMinPFE(logGamma, prob.pFE, prob.min_pFE_ind);
    plotDecisions(D.d20k.x, D.d20k.labels, decision_ideal);

    plotERMContours(D.d20k.x, w0, mu0, Sigma0, w1, mu1, Sigma1, logGamma_ideal);
end
...
for ind = 1:length(DType)
    ...
    lin.discriminantScores = lin.theta' * [ones(1, D.d20k.N); D.d20k.x];
    gamma = 0;
    lin.prob = CalcProb(lin.discriminantScores, gamma, D.d20k.labels, D.d20k.N0, D.d20k.N1, D.d20k.phat);
    ...
    quad.discriminantScores = quad.theta' * quad.xScore;
    gamma = 0;
    quad.prob = CalcProb(quad.discriminantScores, gamma, D.d20k.labels, D.d20k.N0, D.d20k.N1, D.d20k.phat);

    plotDecisions(D.d20k.x, D.d20k.labels, lin.prob.decisions);
    title(sprintf('Data and their Classifier Decisions Against the True Label for Linear Logistic Fit\nProbability of Error = %1.1f%%', 100*lin.prob.pFE));
    
    plotDecisions(D.d20k.x, D.d20k.labels, quad.prob.decisions);
    title(sprintf('Data and their Classifier Decisions Against the True Label for Quadratic Logistic Fit\nProbability of Error = %1.1f%%', 100*quad.prob.pFE));
end
...
function gmm = evalGMM(x, alpha, mu, Sigma)
    ...
    for m = 1:length(alpha)
        gmm = gmm + alpha(m) * evaluateGaussian(x, mu(:, m), Sigma(:, :, m));
    end
end

function probability = CalcProb(discriminantScores, logGamma, labels, N0, N1, phat)
    for ind = 1:length(logGamma)
        probability.decisions = discriminantScores >= logGamma(ind);
        Num_pos(ind) = sum(probability.decisions);
        probability.p10(ind) = sum(probability.decisions == 1 & labels == 0) / N0;
        probability.p11(ind) = sum(probability.decisions == 1 & labels == 1) / N1;
        probability.p01(ind) = sum(probability.decisions == 0 & labels == 1) / N1;
        probability.p00(ind) = sum(probability.decisions == 0 & labels == 0) / N0;
        probability.pFE(ind) = probability.p10(ind) * phat(1) + probability.p01(ind) * phat(2);
    end
end

function g = evaluateGaussian(x, mu, Sigma)
    [n, N] = size(x);
    invSigma = inv(Sigma);
...
function plotContours(x, alpha, mu, Sigma) %#ok<DEFNU>
    figure
...
function plotROC(p10, p11, min_FP, min_TP)
    ...
    hold on;
    plot(min_FP, min_TP, 'ko', 'DisplayName', 'Estimated Min. Error', 'LineWidth', 2);
    xlabel('Probability False Positive');
...
function plotDecisions(x, labels, decisions)
    ...
    axis equal;
    grid on;
    title('Data and their classifier decisions versus the true labels');
    xlabel('x_1');
    ylabel('x_2');
...
function plotMinPFE(logGamma, pFE, min_pFE_ind)
    ...
    hold on;
    plot(logGamma(min_pFE_ind), pFE(min_pFE_ind), 'ko', 'DisplayName', 'Minimum Error', 'LineWidth', 2);
    xlabel('Gamma');
...
function plotERMContours(x, alpha0, mu0, Sigma0, alpha1, mu1, Sigma1, logGamma_ideal)
    ...
    lgd=legend('Correct decisions for the data from Class 0', 'Wrong decisions for the data from Class 0', 'Wrong decisions for data from Class 1', 'Correct decisions for data from Class 1', 'Equilevel contours for the discriminant function');
